...
form1 = st.sidebar.form("my_form1",clear_on_submit=True)
#st.sidebar.title("龍華國小傷病管理系統")
form1.title("1.填寫基本資料")
grade=form1.selectbox('年級',range(0,7))
classes=form1.selectbox('班級',range(0,16))
numbers=form1.selectbox('座號',range(0,35))
basic_data=str(grade)+str(classes).zfill(2)+str(numbers).zfill(2)
#body_temperature = None
#obseravtion_time=None
#get_hurt_places=None
if form1.form_submit_button("輸入完畢 下一步"):
    if not basic_data+"\n" in stu_list:
...
st.sidebar.image("https://pic.pimg.tw/c41666/1560907397-2167670633_n.png",caption='身體部位圖')
